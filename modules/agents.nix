      ...
      buildInlinedSkill
        {
          ...
          pointerText = ''
            ...
            - **In OpenCode**: Emulate parallel execution by performing 8 distinct independent passes over the diff using the prompt definitions defined below and merge their results into the final summary format specified below. (OpenCode's `task` tool currently runs batched calls sequentially rather than truly in parallel — see anomalyco/opencode#14195 — so real subagent dispatch there would only add the cost of re-sending the full diff 8 times with no wall-clock benefit; revisit once that's fixed upstream.)
          '';
        }
